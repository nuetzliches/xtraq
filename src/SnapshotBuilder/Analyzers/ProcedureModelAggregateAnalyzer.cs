...
internal static class ProcedureModelAggregateAnalyzer
{
    ...
    public static Dictionary<string, AggregateSummary> CollectAggregateSummaries(TSqlFragment? fragment)
    {
        ...
        DebugOutputHelper.WriteDebug($"[agg-debug] fragment={fragment.GetType().Name}");
        var visitor = new AggregateVisitor();
        fragment.Accept(visitor);
        visitor.ResolvePendingColumns();

        DebugOutputHelper.WriteDebug($"[agg-debug] collected={visitor.Columns.Count}");
    ...
    private sealed class AggregateVisitor : TSqlFragmentVisitor
    {
        ...
        {
            DebugOutputHelper.WriteDebug("[agg-debug] visiting script");
            node.AcceptChildren(this);
        }

        public override void ExplicitVisit(TSqlBatch node)
        {
            DebugOutputHelper.WriteDebug("[agg-debug] visiting batch");
            node.AcceptChildren(this);
        }

        public override void ExplicitVisit(CreateProcedureStatement node)
        {
            DebugOutputHelper.WriteDebug("[agg-debug] visiting create procedure");
            node.AcceptChildren(this);
        }

        public override void ExplicitVisit(SelectStatement node)
        {
            DebugOutputHelper.WriteDebug("[agg-debug] visiting select statement");
            node.AcceptChildren(this);
        }
        ...
        public override void ExplicitVisit(QuerySpecification node)
        {
            ...
            DebugOutputHelper.WriteDebug($"[agg-debug] enter QuerySpecification alias={context.DerivedAlias ?? "<root>"} isRoot={isRoot}");
            node.AcceptChildren(this);
        ...
        public override void ExplicitVisit(SelectScalarExpression node)
        {
            ...
            {
                DebugOutputHelper.WriteDebug($"[agg-debug-skip] stack={_contextStack.Count} subDepth={_scalarSubqueryDepth} expr={node.Expression?.GetType().Name}");
            ...
            DebugOutputHelper.WriteDebug($"[agg-debug] alias={alias} isTop={current.IsTopLevel} isAgg={info.IsAggregate} expr={node.Expression?.GetType().Name}");
        ...
        private AggregateInfo Analyze(ScalarExpression expression)
        {
            ...
            return expression switch
            {
                ...
                CastCall castCall => AnalyzeCast(castCall),
        ...
        private AggregateInfo AnalyzeCast(CastCall castCall)
        {
            var result = Analyze(castCall.Parameter);
            var normalized = NormalizeDataType(castCall.DataType);
            if (!string.IsNullOrWhiteSpace(normalized))
            {
                result = result with { SqlTypeName = normalized };
            }
        ...
        private AggregateInfo AnalyzeConvert(ConvertCall convertCall)
        {
            ...
            var normalized = NormalizeDataType(convertCall.DataType);
            if (!string.IsNullOrWhiteSpace(normalized))
            {
                result = result with { SqlTypeName = normalized };
            }
        ...
        private AggregateInfo AnalyzeFunctionCall(FunctionCall functionCall)
        {
            ...
            if (AggregateFunctionCatalog.IsAggregateName(name))
            {
                var normalized = AggregateFunctionCatalog.NormalizeName(name)!;
                var combined = AggregateInfo.ForAggregate(normalized);
                ...
                {
                    combined = combined with { SqlTypeName = InferAggregateType(normalized, combined.HasIntegerLiteral, combined.HasDecimalLiteral) };
                }
        ...
        private static string? InferAggregateType(string functionName, bool hasIntegerLiteral, bool hasDecimalLiteral)
        {
            return functionName switch
            {
                ...
                "sum" => hasDecimalLiteral
                    ? "decimal(38,10)"
                    : hasIntegerLiteral ? "int" : "decimal(38,10)",
                _ => null
            };
        }
        ...
        private static string? NormalizeDataType(DataTypeReference? dataType)
        {
            if (dataType == null)
            {
                return null;
            ...
            {
                SqlDataTypeReference sqlDataType => FormatSqlDataType(sqlDataType),
                ParameterizedDataTypeReference parameterized => FormatParameterizedDataType(parameterized),
                _ => null
            };
        }

        private static string? FormatSqlDataType(SqlDataTypeReference sqlDataType)
        {
            var baseName = NormalizeSqlTypeOption(sqlDataType.SqlDataTypeOption);
            if (string.IsNullOrWhiteSpace(baseName))
            {
                return null;
            }

            if (sqlDataType.Parameters is not { Count: > 0 })
            {
                return baseName;
            }
            ...
            {
                var precision = TryParseInt(sqlDataType.Parameters[0]);
                if (!precision.HasValue)
                {
                    return baseName;
                }

                var scale = sqlDataType.Parameters.Count > 1
                    ? TryParseInt(sqlDataType.Parameters[1]) ?? 0
                    : 0;

                return string.Concat(baseName, "(", precision.Value.ToString(CultureInfo.InvariantCulture), ",", scale.ToString(CultureInfo.InvariantCulture), ")");
            }

            if (baseName is "float")
            {
                var bits = TryParseInt(sqlDataType.Parameters[0]);
                if (!bits.HasValue)
                {
                    return baseName;
                }

                return string.Concat(baseName, "(", bits.Value.ToString(CultureInfo.InvariantCulture), ")");
            }

            return baseName;
        }

        private static string? FormatParameterizedDataType(ParameterizedDataTypeReference parameterized)
        {
            if (parameterized is SqlDataTypeReference sqlDataType)
            {
                return FormatSqlDataType(sqlDataType);
            }

            var name = parameterized.Name?.BaseIdentifier?.Value;
            ...
            if (parameterized.Parameters is not { Count: > 0 })
            {
                return normalized;
            }
            ...
            {
                if (parameter is not Literal literal)
                {
                    continue;
                }
        ...
        private static int? TryParseInt(Literal? literal)
        {
            ...
            return int.TryParse(literal.Value, NumberStyles.Integer, CultureInfo.InvariantCulture, out var parsed)
                ? parsed
                : null;
        }
