...
description: The generator deterministically replaces generic placeholder names (`ResultSet1`, `ResultSet2`, …) with meaningful, table-based names. The resolver is always-on (no feature flag) and optimized for stability over aggressiveness.
---
...
Roadmap tracking for result-set naming now lives in the central checklist ([Roadmap](/roadmap)).
