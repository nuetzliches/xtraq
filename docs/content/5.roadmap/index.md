...
- [ ] Finish the schema cache invalidation rollout by covering drop scenarios, chained dependency updates, and persistence rehydration with integration tests for `SchemaObjectCacheManager` and related orchestrators.
- [ ] Refactor the interactive command palette (`CommandPalettePrototype`) into dedicated command handlers with shared UX primitives so CLI flows trim repetitive prompts and surface clearer defaults.
- [ ] Consolidate CLI logging and telemetry messaging into levelled summaries that keep verbose output opt-in while providing concise success/failure reports by default.
- [ ] Publish a CLI UX style guide in `docs/` that standardises prompt wording, validation messages, and confirmation flows for future features.
