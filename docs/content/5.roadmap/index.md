...
- [ ] Modularise the CLI host. `Program.RunCliAsync` in `src/Program.cs` now exceeds 1,200 lines and mixes DI bootstrapping, environment probing, parser wiring, telemetry, and option normalization. Break the entry point into the command catalog/option builder described in `src/Cli/README.md` so shared logic is reusable and testable.
  - Planned steps: (1) extract the environment/bootstrap logic into `CliEnvironmentBootstrapper` surfaced via `Xtraq.Cli.Hosting`, (2) introduce a `CliHostBuilder` that wires configuration + DI and exposes `CliHostContext`, (3) move option/command definitions into a reusable `CliCommandAppBuilder`, (4) encapsulate command execution + telemetry into a dedicated `CliCommandExecutor` so Program.cs becomes a thin shim that resolves the host and delegates to the builder output.
- [x] Validate `--procedure` filters at the parser boundary. `src/Program.cs` now normalizes and deduplicates filters, accepts `*`/`?` wildcards, and surfaces invalid tokens before the CLI executes. `ProgramProcedureOptionTests` documents the accepted syntax.
