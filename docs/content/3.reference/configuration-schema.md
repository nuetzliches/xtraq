...
The modern CLI relies on a dual-file model that centres on the tracked `.xtraqconfig` snapshot for non-sensitive defaults. Secrets stay outside of source control (typically in `.env` or environment variables). Legacy JSON configuration (`xtraq.json`) is ignored and remains in the repository only for historical reference. This page captures the steady-state precedence and supported keys.
...
`.xtraqconfig` is the canonical, versioned baseline that keeps namespaces, output directories, and allow-lists in sync across the team. Secrets can override values via environment variables or `.env`. If a value is not present in any layer the CLI uses built-in defaults. No generator behavior reads configuration values from legacy JSON files.
...
| `XTRAQ_BUILD_SCHEMAS`                   | Comma-separated allow list of schemas to include during generation | Empty list → all schemas except system-reserved ones |
...
- `.xtraqconfig` is generated from the supplied inputs and **must** be checked in so CI and contributors share the same defaults.
