...
/// <summary>Minimal API extension for '[sample].[SyncUserContacts]'.</summary>
public static class SyncUserContactsRouteHandlerBuilderExtensions
{
    public static RouteHandlerBuilder WithSyncUserContactsProcedure(this RouteHandlerBuilder builder)
    {
        ArgumentNullException.ThrowIfNull(builder);

        return builder.WithProcedure<SyncUserContactsRequest, SyncUserContactsResult>(
            static pipeline => pipeline.WithExecutor(
                static async ValueTask<SyncUserContactsResult> (dbContext, request, cancellationToken) =>
                {
                    var bindingProvider = dbContext as IXtraqParameterBindingProvider;
                    var input = await SyncUserContactsRequestMapper.ToInputAsync(request, bindingProvider, cancellationToken).ConfigureAwait(false);
                    return await SyncUserContactsExtensions.SyncUserContactsAsync(dbContext, input, cancellationToken).ConfigureAwait(false);
                }));
    }
}
