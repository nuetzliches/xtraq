...
- ✅ **Feasible without template changes.** Builders can live in application code, wrap generated `IXtraqDbContext` extensions, and remain testable.
- ⚠️ **Avoid hiding streaming APIs.** If a procedure exposes a `StreamResult…Async` helper, keep builder overloads that delegate to the streaming pipeline; buffering everything defeats the generator’s opt-in streaming story.
- 🔄 **Consider DI scoping.** Builders should stay cheap to allocate and respect the existing `IXtraqDbContext` lifetime to avoid connection leaks.
- 📄 **Document intent.** Add XML comments on custom builders so team members know they sit *outside* the generator and are safe to evolve.
