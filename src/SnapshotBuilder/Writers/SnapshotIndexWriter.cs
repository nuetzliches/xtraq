...
internal sealed class SnapshotIndexWriter
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var proc in updated ?? Array.Empty<ProcedureAnalysisResult>())
        {
            ...
            var entry = new IndexProcedureEntry
            {
            ...
            // Add result set metadata for offline build support
            if (proc?.Procedure?.ResultSets != null && proc.Procedure.ResultSets.Count > 0)
            {
                entry.ResultSets = BuildResultSetMetadata(proc.Procedure.ResultSets);
            }
        ...
        if ((schemaArtifacts?.UserDefinedTypes == null || schemaArtifacts.UserDefinedTypes.Count == 0) && existing?.UserDefinedTypes != null)
        {
            ...
            {
                if (!IsValidUserDefinedTypeEntry(entry))
                {
                    continue;
                }

                userDefinedTypeEntries[BuildUdtKey(entry)] = entry;
            }
        }
        ...
        var userDefinedTypeList = userDefinedTypeEntries.Values
            .Where(e => !string.IsNullOrWhiteSpace(e.File))
            .OrderBy(e => e.Catalog, StringComparer.OrdinalIgnoreCase)
        ...
        fingerprintParts.AddRange(userDefinedTypeList.Select(t =>
        {
            var prefix = string.IsNullOrWhiteSpace(t.Catalog) ? string.Empty : string.Concat(t.Catalog, ".");
            return $"udt:{prefix}{t.Schema}.{t.Name}:{t.Hash}";
        }));
    ...
    private static List<IndexResultSetEntry> BuildResultSetMetadata(IReadOnlyList<ProcedureResultSet> resultSets)
    {
        var metadata = new List<IndexResultSetEntry>();

        foreach (var resultSet in resultSets)
        {
            if (resultSet == null) continue;

            var entry = new IndexResultSetEntry
            {
                ReturnsJson = resultSet.ReturnsJson,
                ReturnsJsonArray = resultSet.ReturnsJsonArray,
                JsonRootProperty = resultSet.JsonRootProperty,
                HasSelectStar = resultSet.HasSelectStar,
                ProcedureRef = BuildProcedureRef(resultSet)
            };

            if (resultSet.Columns != null && resultSet.Columns.Count > 0)
            {
                entry.Columns = BuildColumnMetadata(resultSet.Columns);
            }
    ...
    private static List<IndexColumnEntry> BuildColumnMetadata(IReadOnlyList<ProcedureResultColumn> columns)
    {
        var metadata = new List<IndexColumnEntry>();
        ...
        foreach (var column in columns)
        {
            ...
            var entry = new IndexColumnEntry
            {
                ...
                IsNullable = (column.IsNullable == true) || (column.ForcedNullable == true),
                ...
                ReturnsJson = column.ReturnsJson ?? false,
                ReturnsJsonArray = column.ReturnsJsonArray ?? false,
                IsNestedJson = column.IsNestedJson ?? false,
                JsonRootProperty = column.JsonRootProperty,
                JsonElementClrType = column.JsonElementClrType,
                JsonElementSqlType = column.JsonElementSqlType,
            ...
            {
                entry.Columns = BuildColumnMetadata(column.Columns);
            }
    ...
    private static string? BuildProcedureRef(ProcedureResultSet resultSet)
    {
        if (resultSet?.Reference != null && resultSet.Reference.Kind == ProcedureReferenceKind.Procedure)
        {
            return SnapshotWriterUtilities.ComposeSchemaObjectRef(resultSet.Reference.Schema, resultSet.Reference.Name);
        }
    ...
    private static bool IsValidUserDefinedTypeEntry(IndexUserDefinedTypeEntry? entry)
        => entry != null && !string.IsNullOrWhiteSpace(entry.Schema) && !string.IsNullOrWhiteSpace(entry.Name);

    private static string BuildUdtKey(IndexUserDefinedTypeEntry entry)
    {
        var baseKey = SnapshotWriterUtilities.BuildKey(entry.Schema, entry.Name);
        if (string.IsNullOrWhiteSpace(entry.Catalog))
        {
            return baseKey;
        }

        return string.Concat(entry.Catalog.Trim(), ".", baseKey);
    }
}
