...
4. `.xtraqconfig.local` (optional overlay for machine-local defaults)
5. `.xtraqconfig` (canonical team defaults)
6. Built-in defaults baked into the CLI

Although `.env` is higher in the precedence chain, prefer storing any non-secret value in `.xtraqconfig`. That way new machines or CI agents do not depend on copying a local `.env` file. Use `.xtraqconfig.local` sparingly for experiments or workstation-specific defaults that should never leave your machine.

## Editing `.xtraqconfig`

`xtraq init` writes `.xtraqconfig` automatically, but you can edit it directly to capture project-wide changes. Every property in the file is optional—you can omit the entire object and rely on the defaults, or override only the values that need to differ. When `Namespace` is absent, the CLI infers it from the consuming project/assembly; when `OutputDir` is missing, the generator defaults to `Xtraq`. When every value uses the built-in defaults, `xtraq init` leaves the file as `{}` instead of emitting redundant properties.
...
| `XTRAQ_NO_UPDATE`                       | Disables auto-update prompts when set                            | Prompts remain enabled when unset; same as `--no-update` |
