...
internal sealed class LegacySnapshotBridge
{
    ...
    private Dictionary<string, List<SnapshotInput>> LoadLegacyFallbackSnapshot(string currentFingerprint)
    {
        ...
        try
        {
            ...
            var cacheRoot = Path.Combine(projectRoot, ".xtraq", "cache");
            var legacyCacheRoot = Path.Combine(projectRoot, ".xtraq", "cache", "schema");
            var legacySchemaRoot = Path.Combine(projectRoot, ".xtraq", "snapshots");
