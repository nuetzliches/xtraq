...
#if NET8_0_OR_GREATER && XTRAQ_API_MODE_MINIMAL
/// <summary>Minimal API extension for '[sample].[UserOrderInsights]'.</summary>
public static class UserOrderInsightsRouteHandlerBuilderExtensions
{
    public static RouteHandlerBuilder WithUserOrderInsightsProcedure(this RouteHandlerBuilder builder)
    {
        ArgumentNullException.ThrowIfNull(builder);

        return builder.WithProcedure<UserOrderInsightsRequest, UserOrderInsightsResult>(
            static pipeline => pipeline.WithExecutor(
                static async ValueTask<UserOrderInsightsResult> (dbContext, request, cancellationToken) =>
                {
                    var bindingProvider = dbContext as IXtraqParameterBindingProvider;
                    var input = await UserOrderInsightsRequestMapper.ToInputAsync(request, bindingProvider, cancellationToken).ConfigureAwait(false);
                    return await UserOrderInsightsExtensions.UserOrderInsightsAsync(dbContext, input, cancellationToken).ConfigureAwait(false);
                }));
    }
}
#endif
