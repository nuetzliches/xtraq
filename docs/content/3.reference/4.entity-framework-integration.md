...
Entity Framework support is disabled by default so projects without EF Core packages continue compiling cleanly. Enable the adapter before running `xtraq build` and define the compilation symbol in the consuming project:
...
<PropertyGroup>
  <DefineConstants>$(DefineConstants);XTRAQ_ENTITY_FRAMEWORK_ENABLED</DefineConstants>
</PropertyGroup>
```

- CLI toggle: `xtraq build --entity-framework` sets the flag for the current invocation.
- Use `--entity-framework` (CLI switch) or set `EntityFramework.Enabled` inside `.xtraqconfig`/`.xtraqconfig.local` to toggle the feature. Environment variables are no longer consumed for this setting.

## Registering the Integration

Call the extension after registering your DbContext and the generated Xtraq services. You normally do **not** need to set `ConnectionString` manually – the adapter will reuse EF’s connection string and ambient transaction. Provide `ConnectionString` only when you want Xtraq to use a different database.
