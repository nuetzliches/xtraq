...
public static class UpdateUserBioRouteHandlerBuilderExtensions
{
    public static RouteHandlerBuilder WithUpdateUserBioProcedure(this RouteHandlerBuilder builder)
    {
        ...
        return builder.WithProcedure<UpdateUserBioRequest, UpdateUserBioResult>(
            static pipeline => pipeline.WithExecutor(
                static async ValueTask<UpdateUserBioResult> (dbContext, request, cancellationToken) =>
                {
                    var bindingProvider = dbContext as IXtraqParameterBindingProvider;
                    var input = await UpdateUserBioRequestMapper.ToInputAsync(request, bindingProvider, cancellationToken).ConfigureAwait(false);
                    return await UpdateUserBioExtensions.UpdateUserBioAsync(dbContext, input, cancellationToken).ConfigureAwait(false);
                }));
    }
}
#endif
