...
internal sealed class SchemaSnapshotFileLayoutService
{
    ...
    private string? EnsureBaseDir()
    {
        ...
        var baseDir = Path.Combine(working, ".xtraq", "snapshots");
    ...
    public void SaveExpanded(SchemaSnapshot snapshot)
    {
        ...
        {
            var fileName = $"{Xtraq.Utils.NameSanitizer.SanitizeForFile(proc.Schema)}.{Xtraq.Utils.NameSanitizer.SanitizeForFile(proc.Name)}.json";
            ...
            // Forwarding minimalism: strip JSON flags and columns from result sets that reference ExecSourceProcedureName
        ...
        var procedureFilter = Environment.GetEnvironmentVariable("XTRAQ_BUILD_PROCEDURES");
        ...
        {
            var fileName = $"{Xtraq.Utils.NameSanitizer.SanitizeForFile(udtt.Schema)}.{Xtraq.Utils.NameSanitizer.SanitizeForFile(udtt.Name)}.json";
            var path = Path.Combine(ttDir, fileName);
            // Remove hash information from the snapshot so only the functional structure is persisted
        ...
        {
            var fileName = $"{Xtraq.Utils.NameSanitizer.SanitizeForFile(fn.Schema)}.{Xtraq.Utils.NameSanitizer.SanitizeForFile(fn.Name)}.json";
            var path = Path.Combine(fnDir, fileName);
            // IsTableValued: prune false values (persist true only)
            if (fn.IsTableValued == false) fn.IsTableValued = null;
            // Remove return fields for scalar JSON functions
            if (fn.ReturnsJson == true)
            {
                fn.ReturnSqlType = null; // do not persist
                fn.ReturnIsNullable = null;
                // Keep ReturnMaxLength for now (helps distinguish NVARCHAR(MAX) vs NVARCHAR(200)); optionally prune later
            }
            // Empty columns list -> null
            ...
            void PruneFnColumns(List<SnapshotFunctionColumn> cols)
            {
                ...
                foreach (var c in cols.ToList())
                {
                    ...
                    if (c.IsNestedJson == false) c.IsNestedJson = null; // persist true only
        ...
        {
            var baseName = Xtraq.Utils.NameSanitizer.SanitizeForFile(udt.Name);
            var schemaName = Xtraq.Utils.NameSanitizer.SanitizeForFile(udt.Schema);
            var catalogName = string.IsNullOrWhiteSpace(udt.Catalog) ? null : Xtraq.Utils.NameSanitizer.SanitizeForFile(udt.Catalog);
            if (string.IsNullOrWhiteSpace(catalogName)) catalogName = null;
            // Base type file (catalog.schema.name.json if catalog provided)
            var fileName = BuildTypeFileName(catalogName, schemaName, baseName);
            var path = Path.Combine(typesDir, fileName);
            var json = JsonSerializer.Serialize(udt, _jsonOptions);
            var newHash = HashUtils.Sha256Hex(json).Substring(0, 16);
            File.WriteAllText(path, json); // delta optimization not needed for these small files
            typeHashes.Add(new FileHashEntry { Catalog = udt.Catalog, Schema = udt.Schema, Name = udt.Name, File = fileName, Hash = newHash });
            ...
            try
            {
                ...
                var fileNameUnderscore = BuildTypeFileName(catalogName, schemaName, underscoreName);
                ...
                typeHashes.Add(new FileHashEntry { Catalog = udt.Catalog, Schema = udt.Schema, Name = underscoreName, File = fileNameUnderscore, Hash = hashUnderscore });
            }
            catch { }
        }
        ...
        {
            var fileName = $"{Xtraq.Utils.NameSanitizer.SanitizeForFile(tbl.Schema)}.{Xtraq.Utils.NameSanitizer.SanitizeForFile(tbl.Name)}.json";
            var path = Path.Combine(tablesDir, fileName);
            // Column pruning similar to procedure columns: remove false/null values
        ...
        {
            var fileName = $"{Xtraq.Utils.NameSanitizer.SanitizeForFile(vw.Schema)}.{Xtraq.Utils.NameSanitizer.SanitizeForFile(vw.Name)}.json";
        ...
        var index = new ExpandedSnapshotIndex
        {
            ...
            Parser = snapshot.Parser ?? new SnapshotParserInfo(),
            Stats = snapshot.Stats ?? new SnapshotStats(),
            Procedures = procHashes,
            TableTypes = ttHashes,
            FunctionsVersion = snapshot.FunctionsVersion,
            Functions = fnHashesOrdered,
            UserDefinedTypes = typeHashes
                    .OrderBy(h => h.Catalog, StringComparer.OrdinalIgnoreCase)
                    .ThenBy(h => h.Schema, StringComparer.OrdinalIgnoreCase)
    ...
    private static string BuildTypeFileName(string? catalog, string schema, string name)
    {
        var segments = new List<string>(3);
        if (!string.IsNullOrWhiteSpace(catalog)) segments.Add(catalog);
        if (!string.IsNullOrWhiteSpace(schema)) segments.Add(schema);
        if (!string.IsNullOrWhiteSpace(name)) segments.Add(name);
        if (segments.Count == 0)
        {
            return "type.json";
        }
            ...
            using (var writer = new Utf8JsonWriter(ms, new JsonWriterOptions { Indented = true }))
            {
                ...
                foreach (var prop in root.EnumerateObject())
                {
                    if (prop.NameEquals("ResultSets"))
                    {
                        ...
                        {
                            // Detect a pure placeholder: ExecSource* set, no columns, no ReturnsJson flag
                            ...
                            {
                                // Write local or forwarded real sets with extra pruning logic applied
                                writer.WriteStartObject();
    ...
    public SchemaSnapshot? LoadExpanded()
    {
        ...
        try
        {
            ...
            index = JsonSerializer.Deserialize<ExpandedSnapshotIndex>(indexJson, _jsonOptions);
        }
        catch
        {
            return null;
        ...
        var snapshot = new SchemaSnapshot
        {
            ...
            Fingerprint = index.Fingerprint ?? string.Empty,
            Parser = index.Parser,
            Stats = index.Stats,
            ...
            // Important: schemas are not currently reconstructed from index.json - we derive them later.
        ...
        {
            if (p == null || string.IsNullOrWhiteSpace(p.File))
            {
                continue;
            }
            ...
            try
            {
                ...
                foreach (var pi in parameterList)
                {
                    if (pi == null)
                    {
                        continue;
                    ...
                    if (nameSegment != null && string.IsNullOrWhiteSpace(pi.TableTypeName))
                    {
                        pi.TableTypeName = nameSegment;
                    }
                }
        ...
        {
            if (t == null || string.IsNullOrWhiteSpace(t.File))
            {
                continue;
            }
        ...
        {
            if (typeEntry == null || string.IsNullOrWhiteSpace(typeEntry.File))
            {
                continue;
            }
        ...
        {
            if (tableEntry == null || string.IsNullOrWhiteSpace(tableEntry.File))
            {
                continue;
            }
        ...
        {
            if (viewEntry == null || string.IsNullOrWhiteSpace(viewEntry.File))
            {
                continue;
            }
        ...
        if (index.FunctionsVersion.HasValue)
        {
            ...
            {
                if (f == null || string.IsNullOrWhiteSpace(f.File))
                {
                    continue;
                }
        ...
        // Derive schema names by unifying procedure, UDTT, table, and view schemas
        try
        {
            ...
            foreach (var sn in schemaNames)
            {
                ...
                {
                    Name = sn,
                    // Status is not persisted; IgnoredSchemas drives later filtering.
                });
            }
        }
        catch { /* best effort */ }
    ...
    private static ParameterFileModel BuildParameterFileModel(SnapshotInput input)
    {
        ...
        var normalizedTableTypeRef = TableTypeRefFormatter.Normalize(input.TableTypeRef);
        if (string.IsNullOrEmpty(normalizedTableTypeRef))
        {
            normalizedTableTypeRef = TableTypeRefFormatter.Normalize(CombineTypeRef(input.TableTypeSchema, input.TableTypeName));
        }
        ...
        if (!string.IsNullOrWhiteSpace(parameter.TableTypeRef))
        {
    ...
    private static ParameterTypeRefKind DetermineTypeRefKind(string baseDir, string? schema, string? name, bool? legacyHint)
    {
        ...
        var sanitizedName = Xtraq.Utils.NameSanitizer.SanitizeForFile(name);
    ...
    public sealed class ExpandedSnapshotIndex
    {
        ...
        public SnapshotParserInfo Parser { get; set; } = new();
