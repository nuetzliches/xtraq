...
- JSON enrichment diagnostics now follow the regular log level; enable `XTRAQ_LOG_LEVEL=debug` (optionally combine with `XTRAQ_JSON_AST_DIAG=1`) whenever detailed ScriptDom traces are needed. The former `project.jsonTypeLogLevel` toggle was retired with the previous enricher.
...
> For endpoint integration patterns and DTO mapping guidance (applicable to JSON and tabular procedures alike), see [Fluent Pipelines & DTO Mapping](/reference/api-integration).
