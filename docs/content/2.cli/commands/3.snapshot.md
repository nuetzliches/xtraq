---
title: snapshot
description: Synchronizes stored procedures, parameters, and schema metadata into `.xtraq/` using the connection defined in your `.env` file.
aiTags: ["cli", "snapshot"]
---
...
The remaining global switches (such as `--debug`, `--telemetry`, and `--ci`) behave exactly as described in the [CLI overview](/cli).
