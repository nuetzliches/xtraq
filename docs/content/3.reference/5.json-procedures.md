...
- Pass `--json-include-null-values` when running `xtraq build`, or set `ResultSet.Json.IncludeNullValues` to `true` inside `.xtraqconfig`/`.xtraqconfig.local`, whenever the project needs `[JsonIncludeNullValues]` attributes. Treat CLI overrides as short-lived exceptions rather than the norm.
