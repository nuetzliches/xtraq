...
public static class Program
{
    ...
    public static async Task<int> RunCliAsync(string[] args)
    {
            ...
            bool defaultRefresh)
        {
            ...
            try
            {
                invocationContext.ExitCode = await command.ExecuteAsync(commandContext, invocationContext.GetCancellationToken()).ConfigureAwait(false);
            }
            finally
            {
        ...
        {
            await ExecuteCommandAsync(invocationContext, buildDescriptor, buildCommandHandler, null, null, defaultRefresh: true);
        });
        ...
        {
            await ExecuteCommandAsync(invocationContext, snapshotDescriptor, snapshotCommandHandler, snapshotProjectArgument, null, defaultRefresh: false);
        });
        ...
        updateCommand.SetHandler(async context =>
        {
            ...
            commandOptionsAccessor.Update(options);
            context.ExitCode = CommandResultMapper.Map(await runtime.UpdateAsync(options).ConfigureAwait(false));
        });
