...
- Options:
  ...
  - `--namespace`, `-n`: override the auto-resolved namespace (persisted to `.xtraqconfig`).
  - `--connection`, `-c`: seed `XTRAQ_GENERATOR_DB` inside `.env`.
  - `--schemas`, `-s`: capture the `BuildSchemas` array in `.xtraqconfig` (comma-separated allow-list input).
