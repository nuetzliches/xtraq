...
| `ResultSets[i].Columns[j].JsonIncludeNullValues` | Column     | Column-level `INCLUDE_NULL_VALUES` flag is preserved to mirror the SQL projection.    |
...
| `ResultSets[i].Columns[j].Alias`                 | Column     | Direct `AS ...` alias from SQL (if absent the analyzer infers one).                   |
| `ResultSets[i].Columns[j].SourceColumn`          | Column     | Raw table column name persisted alongside `Name` when the projection is aliased.      |
| `ResultSets[i].Columns[j].TypeRef`               | Column     | Normalised system-type reference (for example `sys.nvarchar`) used for codegen.       |
| `ResultSets[i].Columns[j].UserTypeRef`           | Column     | Fully-qualified user-defined type reference when the column binds to a UDTT/UDT.      |
| `ResultSets[i].Columns[j].SqlTypeName`           | Column     | Friendly SQL type description (includes casts, precision/scale where applicable).     |
| `ResultSets[i].Columns[j].IsNullable`            | Column     | `true` when the analyzer or schema metadata proves the column can be `NULL`.          |
| `ResultSets[i].Columns[j].MaxLength`             | Column     | Emitted when the underlying SQL type exposes a length (varchar, nvarchar, etc.).      |
| `ResultSets[i].Columns[j].JsonElementClrType`    | Column     | CLR type of JSON elements inside arrays (when we can infer them).                     |
| `ResultSets[i].Columns[j].JsonElementSqlType`    | Column     | SQL type of JSON elements inside arrays (when we can infer them).                     |
| `ResultSets[i].Columns[j].ReturnsUnknownJson`    | Column     | Analyzer could not determine the nested JSON shape; generator treats it as opaque.    |
| `ResultSets[i].Columns[j].Columns`               | Column     | Nested column collection for multi-level JSON hierarchies.                            |

`SourceColumn` makes it possible to reconcile deterministic aliases with their originating table columns without changing the meaning of the `Name` field that downstream code generators already consume.

### Name vs. Alias vs. SourceColumn

- `Name` stays the canonical identifier that the generator uses for DTO properties. It may be inferred, normalised, or de-duplicated to keep result sets deterministic.
- `Alias` mirrors the literal `AS SomeAlias` clause (when provided) so advanced tooling can reason about the authoring intent.
- `SourceColumn` preserves the underlying table column even when `Name` and `Alias` diverge (for example dotted aliases or collision suffixes). This is the safest place to look when you need the raw schema binding.
...
- Aliases such as `AS [status.label]` or `AS [account.name]` become dotted names in the snapshot. The generator converts these aliases into nested record structs.
...
- Pass `--json-include-null-values` when running `xtraq build`, or add `XTRAQ_JSON_INCLUDE_NULL_VALUES=1` to `.xtraqconfig`, whenever the project needs `[JsonIncludeNullValues]` attributes. Treat overrides (`.xtraqconfig.local`, environment variables) as short-lived exceptions rather than the norm.
...
- Stored procedures may stream large `FOR JSON` payloads across multiple rows. The generated projectors now concatenate all chunks before deserializing and keep `ResultRawJson` intact, so no payload is truncated.
- Nullable input parameters are bound as `DBNull.Value` when null to avoid SQL errors such as “Parameter ... not supplied”.
...
- JSON enrichment diagnostics now follow the regular log level; enable `XTRAQ_LOG_LEVEL=debug` (optionally combine with `XTRAQ_JSON_AST_DIAG=1`) whenever detailed ScriptDom traces are needed. The former `project.jsonTypeLogLevel` toggle was retired with the legacy enricher.
...
> For endpoint integration patterns and DTO mapping guidance (applicable to JSON and tabular procedures alike), see [Minimal APIs & DTO Mapping](./api-integration.md).
