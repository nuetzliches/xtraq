...
/// 1. If XtraqConfiguration provides an explicit NamespaceRoot (XTRAQ_NAMESPACE) -> return it verbatim.
...
/// 4. (No automatic suffix) – physical output directory (e.g. /Xtraq) provides separation; we don't duplicate in namespace.
...
/// </summary>
internal sealed class NamespaceResolver
{
    private readonly XtraqConfiguration _cfg;
    private readonly Action<string>? _logWarn;

    public NamespaceResolver(XtraqConfiguration cfg, Action<string>? logWarn = null)
    {
        _cfg = cfg;
        _logWarn = logWarn;
    }
    ...
    public string Resolve(string? searchRoot = null)
    {
        ...
        if (!startDir.Exists)
        {
            _logWarn?.Invoke($"[xtraq namespace] searchRoot '{searchRoot}' does not exist. Using fallback.");
            return "";
        }
        ...
        if (!string.IsNullOrWhiteSpace(configPath))
        {
            ...
            {
                _logWarn?.Invoke($"[xtraq namespace] configPath resolution failed: {ex.Message}");
            }
        }
        ...
        {
            baseName = ToPascalCase(effectiveDir.Name);
            _logWarn?.Invoke($"[xtraq namespace] scan error: {ex.Message}");
        }
    ...
    private static string ToPascalCase(string input)
    {
        ...
        if (string.IsNullOrEmpty(candidate)) candidate = "Xtraq";
