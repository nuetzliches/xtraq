---
title: init
description: Initialize a xtraq project with a tracked `.xtraqconfig` snapshot and minimal `.env` secrets. Bootstraps a `.env` file (or updates an existing one) with core XTRAQ\_\* keys and writes a tracked `.xtraqconfig` snapshot for non-sensitive defaults.
---
...
| `-n, --namespace <name>`            | Root namespace                  | `XTRAQ_NAMESPACE`     | Optional override – CLI resolves automatically when omitted  |
