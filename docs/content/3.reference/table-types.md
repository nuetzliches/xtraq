...
xtraq generates strongly-typed records for each user-defined table type (UDTT) discovered in the snapshot. This enables safe passing of structured values (lists / sets) to stored procedures without manual `DataTable` plumbing.
...
When `XTRAQ_BUILD_SCHEMAS` is set (either in `.env` or implicitly via `.xtraqconfig`), only table types in those schemas are generated. This central positive filtering aligns with stored procedure generation (single configuration surface).
