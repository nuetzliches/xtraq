...
internal static class ProcedureModelExecAnalyzer
{
    ...
    public static void Apply(TSqlFragment? fragment, ProcedureModel? model)
    {
        ...
        foreach (var executed in visitor.ExecutedProcedures)
        {
            ...
            var hasPlaceholder = model.ResultSets.Any(rs =>
                rs.Reference != null &&
                string.Equals(rs.Reference.Name, executed.Name, StringComparison.OrdinalIgnoreCase) &&
                string.Equals(rs.Reference.Schema ?? string.Empty, executed.Schema ?? string.Empty, StringComparison.OrdinalIgnoreCase));
            ...
            if (!hasPlaceholder)
            {
                model.ResultSets.Add(new ProcedureResultSet
                {
                    ...
                    {
                        Kind = ProcedureReferenceKind.Procedure,
                        Schema = executed.Schema,
                        Name = executed.Name
                    }
                });
