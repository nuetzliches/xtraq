...
Entity Framework support is disabled by default so projects without EF Core packages continue compiling cleanly. Enable the adapter before running `xtraq build` by setting the tracked configuration flag in `.xtraqconfig`:
...
- [Transaction API roadmap entry](/roadmap#transaction-api)
