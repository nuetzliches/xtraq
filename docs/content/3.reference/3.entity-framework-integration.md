...
<PropertyGroup>
  <DefineConstants>$(DefineConstants);XTRAQ_ENTITY_FRAMEWORK</DefineConstants>
</PropertyGroup>
```

- CLI toggle: `xtraq build --entity-framework` sets the flag for the current invocation.
- Environment variable: `XTRAQ_ENTITY_FRAMEWORK=1` mirrors the `.xtraqconfig` setting when scripting the generator.
