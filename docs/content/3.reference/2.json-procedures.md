...
internal static class FileMappings
{
  ...
  {
    FileId = row.fileId,
    DirectoryRef = row.directoryRef,
    FileNameRef = row.fileNameRef,
    ...
    Storage = new Storage
    {
      StorageId = row.storage.storageId,
      BaseDirectory = row.storage.baseDirectory,
    },
    AvScan = row.avScan?.ToDomain(),
    Record = row.record?.ToDomain(),
    Attributes = row.attributes.Select(ToDomain).ToList(),
  };
