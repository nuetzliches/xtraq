...
The project template in `samples/restapi/Program.cs` demonstrates how any generated procedure—JSON-enabled or purely tabular—plugs into a Minimal API endpoint. Resolve the connection string once, register the generated `IXtraqDbContext`, call the typed helper (for example `FileFindAsync`), and pass the projection straight to the client or a mapper.
...
app.MapGet("/files/{fileId:int}", async Task<Results<Ok<FileResponse>, NotFound>> (int fileId, IXtraqDbContext db, CancellationToken ct) =>
{
  var row = (await db.FileFindAsync(new FileFindInput(fileId), ct).ConfigureAwait(false)).Result;
  return row is null
...
- Need to flow an EF Core transaction? See [Entity Framework Integration](/reference/entity-framework-integration).
- Request DTO overloads exist for procedures with inputs: `db.<Proc>Async(<Proc>Request request, ...)` and `Stream...Async(request, ...)` hydrate missing fields (claims, defaults, TVPs) before execution.

### Fluent Pipeline Endpoint Helpers

Set `Api.Mode` to `Minimal` before running `xtraq build` so the generator emits `ProcedureRouteHandlerBuilderExtensions`, request DTOs, and Minimal API route helpers.
...
- The handler signature stays intact; the pipeline returns the result and bypasses the body.
- Handlers returning `IResult` are respected; otherwise `Results.Ok` is emitted automatically.
- Policies (`WithPolicy`, `WithTransaction`, Labels) mirror non-web contexts.
- Per-procedure helpers (`With…Procedure`) are generated per artifact; no generics needed in route mapping.
...
Generated projects always include `ProcedurePipelineExtensions`; using it is optional. It layers configuration primitives over the typed `IXtraqDbContext` surface without touching the artefacts themselves. The workflow splits into two stages:
...
// Request-based pipeline (mapper uses request DTO + binding provider)
var executionWithRequest = await db.ConfigureProcedure<UserFindRequest, UserFindInput>(
    request,
    static (req, ctx, ct) => UserFindRequestMapper.ToInputAsync(req, ctx as IXtraqParameterBindingProvider, ct));
var resultWithRequest = await executionWithRequest
    .WithExecutor(static (ctx, input, ct) => ctx.UserFindAsync(input, ct))
    .ExecuteAsync(ct);
```
...
5. **Document intent.** Wrap pipeline-based workflows in extension methods with XML summaries so downstream teams know when the fluent surface is being used (the generator always emits it).
