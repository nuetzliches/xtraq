...
| Side effects   | Prompts to bootstrap `.xtraqconfig` if missing, schedules a background update check (skipped via `--no-update`/`XTRAQ_NO_UPDATE`), and when an update is found outside CI it offers to install it. |
