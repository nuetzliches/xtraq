...
- Generated folders `Xtraq/` and `.xtraq/` remain disposable and can be cleared; keep custom code outside of them (see [Generated output is disposable](../../3.reference/1.configuration-schema.md#generated-output-is-disposable)).
...
- JSON null emission: Controlled via `ResultSet.Json.IncludeNullValues` (and `XTRAQ_RESULTSET_JSON_INCLUDE_NULL_VALUES`).
