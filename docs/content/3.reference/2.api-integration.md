...
Enable `Api.Mode` set to `Minimal` before running `xtraq build` so the generator emits `ProcedureRouteHandlerBuilderExtensions` (guarded by `#if NET8_0_OR_GREATER && XTRAQ_API_MODE_MINIMAL`).
...
<PropertyGroup>
  <DefineConstants>$(DefineConstants);XTRAQ_API_MODE_MINIMAL</DefineConstants>
</PropertyGroup>
```
