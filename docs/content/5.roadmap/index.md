...
- [ ] Cache stored procedure metadata per schema so warm snapshot runs cut `StoredProcedureQueries.ObjectLookup` calls by at least 50% (baseline: 1,136) and document the change in telemetry release notes. _(Current implementation still issues per-procedure lookups via `StoredProcedureQueries.ObjectAsync`.)_
