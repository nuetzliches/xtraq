...
- API config: Set `Api.Mode` to `Minimal` when you need request DTOs, table-type request rows, and the associated `Api.Requests.*` auto-bind allow-lists; switch to `None` when a project only requires raw input structs.
