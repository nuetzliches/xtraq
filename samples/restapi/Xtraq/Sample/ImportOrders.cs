...
internal static class ImportOrdersRequestMapper
{
    public static async ValueTask<ImportOrdersInput> ToInputAsync(ImportOrdersRequest? request, IXtraqParameterBindingProvider? bindingProvider, CancellationToken cancellationToken = default)
    {
        ...
        if (!HasValue(Orders))
        {
            Orders = await ResolveTableAsync<OrderImportTableType>(bindingProvider, "[sample].[ImportOrders]", "@Orders", Orders, cancellationToken).ConfigureAwait(false);
        }
        if (!HasValue(Orders))
        {
...
public static class ImportOrdersRouteHandlerBuilderExtensions
{
    public static RouteHandlerBuilder WithImportOrdersProcedure(this RouteHandlerBuilder builder)
    {
        ...
        return builder.WithProcedure<ImportOrdersRequest, ImportOrdersResult>(
            static pipeline => pipeline.WithExecutor(
                static async ValueTask<ImportOrdersResult> (dbContext, request, cancellationToken) =>
                {
                    var bindingProvider = dbContext as IXtraqParameterBindingProvider;
                    var input = await ImportOrdersRequestMapper.ToInputAsync(request, bindingProvider, cancellationToken).ConfigureAwait(false);
                    return await ImportOrdersExtensions.ImportOrdersAsync(dbContext, input, cancellationToken).ConfigureAwait(false);
                }));
    }
}
#endif
