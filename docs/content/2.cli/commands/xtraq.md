---
title: xtraq
description: Default invocation behaviour when no subcommand is supplied.
versionIntroduced: 1.0.0-alpha
experimental: false
authoritative: true
authors: [core]
aiTags: [cli, default, build]
---

# xtraq (default command)

Running the CLI without an explicit subcommand is the fastest way to refresh metadata and generate artefacts in one shot. The root invocation behaves exactly like `xtraq build --refresh-snapshot`.
