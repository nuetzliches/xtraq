...
internal sealed class DatabaseDependencyMetadataProvider : IDependencyMetadataProvider
{
    ...
    private async Task<DateTime?> GetLastModifiedAsync(ProcedureDependency dependency, CancellationToken cancellationToken)
    {
        ...
        var cacheKey = new DependencyKey(dependency.Kind, schema, dependency.Name);
        ...
        DateTime? result = dependency.Kind switch
        {
            ProcedureDependencyKind.Procedure => await QueryModifyDateAsync(schema, dependency.Name, new[] { "P" }, cancellationToken).ConfigureAwait(false),
            ProcedureDependencyKind.Function => await QueryModifyDateAsync(schema, dependency.Name, new[] { "FN", "TF", "IF" }, cancellationToken).ConfigureAwait(false),
            ProcedureDependencyKind.View => await QueryModifyDateAsync(schema, dependency.Name, new[] { "V" }, cancellationToken).ConfigureAwait(false),
            ProcedureDependencyKind.Table => await QueryModifyDateAsync(schema, dependency.Name, new[] { "U" }, cancellationToken).ConfigureAwait(false),
            ProcedureDependencyKind.UserDefinedTableType => await QueryTableTypeModifyDateAsync(schema, dependency.Name, cancellationToken).ConfigureAwait(false),
            ProcedureDependencyKind.UserDefinedType => await QueryUserDefinedTypeModifyDateAsync(schema, dependency.Name, cancellationToken).ConfigureAwait(false),
            _ => null
        };

        _modifyDateCache[cacheKey] = result;
        return result;
    }
