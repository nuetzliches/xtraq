...
namespace Xtraq.Metadata
{
    ...
    /// <summary>
    /// Snapshot metadata provider backed by the local .xtraq/snapshots directory.
    /// </summary>
    public class SchemaMetadataProvider : ISchemaMetadataProvider
    {
        ...
        public SchemaMetadataProvider(string? projectRoot = null)
            : this(projectRoot, console: null, layoutService: null, enhancedSchemaMetadataProvider: null)
        {
        }

        /// <summary>
        /// Internal constructor enabling dependency injection scenarios within the assembly.
        /// </summary>
        internal SchemaMetadataProvider(
            string? projectRoot,
            ...
            IEnhancedSchemaMetadataProvider? enhancedSchemaMetadataProvider)
        {
            ...
            var consoleToUse = console ?? new ConsoleService(new CommandOptions());
            var layoutServiceToUse = layoutService ?? new SchemaSnapshotFileLayoutService();
            var enhancedProvider = enhancedSchemaMetadataProvider ?? CreateEnhancedProvider(consoleToUse);

            WarmUpSupportingServices(_projectRoot, consoleToUse, layoutServiceToUse, enhancedProvider);
        }
        ...
        private static void WarmUpSupportingServices(
            string projectRoot,
            IConsoleService console,
            SchemaSnapshotFileLayoutService layoutService,
            IEnhancedSchemaMetadataProvider enhancedSchemaMetadataProvider)
        {
            try
            {
                DirectoryUtils.SetBasePath(projectRoot);
            }
            catch
            {
            ...
            try
            {
                _ = layoutService.LoadExpanded();
            }
            catch (Exception ex)
            {
                console.Verbose($"[schema-metadata] Snapshot layout warm-up failed: {ex.Message}");
            }

            try
            {
                enhancedSchemaMetadataProvider.IsOfflineModeAvailableAsync().GetAwaiter().GetResult();
            }
            catch (Exception ex)
            {
                console.Verbose($"[schema-metadata] Offline metadata probe failed: {ex.Message}");
            }
        }
