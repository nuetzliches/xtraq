...
- Handler-Signatur bleibt; die Pipeline liefert das Ergebnis, der Body wird übersprungen.
- Handler mit `IResult` laufen unverändert; sonst wird automatisch `Results.Ok` erzeugt.
- Policies (`WithPolicy`, `WithTransaction`, Labels) sind identisch zu Nicht-Web-Kontexten.
- Per-Prozedur-Helfer (`With…Procedure`) werden im jeweiligen Artefakt generiert; keine Generics im Route-Mapping nötig.
