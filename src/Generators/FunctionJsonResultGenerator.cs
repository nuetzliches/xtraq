...
/// </summary>
internal sealed class FunctionJsonResultGenerator : GeneratorBase
{
    public FunctionJsonResultGenerator(ITemplateRenderer renderer, ITemplateLoader? loader = null, XtraqConfiguration? cfg = null)
    ...
    public int Generate(string baseNamespace, string baseOutputDir, IReadOnlyList<FunctionJsonDescriptor> descriptors)
    {
                                              ...
                                              .ThenBy(d => d.FunctionName, StringComparer.OrdinalIgnoreCase))
        {
            ...
            const string fallbackTemplate = "{{ HEADER }}\n#nullable enable\nusing System;\nusing System.Collections.Generic;\n\nnamespace {{ Namespace }};\n\n{{#each Types}}\n{{{ this }}}\n\n{{/each}}";
            var content = Templates.RenderTemplate("FunctionJsonResult", model, fallbackTemplate);
            var filePath = Path.Combine(schemaDir, descriptor.RootTypeName + ".cs");
            File.WriteAllText(filePath, content.TrimEnd() + Environment.NewLine);
            written++;
        }
