...
- Automate regeneration in CI so client libraries stay current with every schema change.
...
`xtraq init` writes the connection string into a project-scoped `.env` and records non-sensitive defaults in `.xtraqconfig`.
::
