...
- [ ] Broaden the warm-run delta window. `SchemaInvalidationOrchestrator.GetEarliestCacheTimestampAsync` (src/Services/SchemaInvalidationOrchestrator.cs) falls back to `DateTime.UtcNow.AddDays(-1)` when no cache timestamp exists, so the very first warm run (or any run after deleting `.xtraq/cache`) silently ignores objects older than 24 hours. Persist per-type baselines or default to `DateTime.MinValue` so we never skip legitimate modifications.
- [ ] Modularise the CLI host. `Program.RunCliAsync` in `src/Program.cs` now exceeds 1,200 lines and mixes DI bootstrapping, environment probing, parser wiring, telemetry, and option normalization. Break the entry point into the command catalog/option builder described in `src/Cli/README.md` so shared logic is reusable and testable.
- [ ] Validate `--procedure` filters at the parser boundary. Today `procedureOption` in `src/Program.cs` simply forwards the raw comma-separated string into `XTRAQ_BUILD_PROCEDURES`, so typos (missing schema, whitespace, wildcard characters) only fail deep inside the generator. Add a validator that enforces the `schema.object` shape, deduplicates entries, and surfaces errors immediately.
- [ ] Remove synchronous file IO from `FileManager<TConfig>.Config` (src/Infrastructure/FileManager.cs). The property blocks on `ReadAsync().GetAwaiter().GetResult()`, so every access deserializes JSON on the calling thread and risks deadlocks when invoked from a sync context. Introduce async initialization plus change tracking so config loads are non-blocking and cached.
...
- [ ] Reduce duplicate stored procedure catalog lookups. Snapshot telemetry still executes `StoredProcedureQueries.ObjectLookup` twice as often as `StoredProcedureQueries.InputParameters` (1,136 vs. 568 calls) because every cached procedure incurs a catalog probe. Cache the core metadata per schema or persist it between runs so no-op procedures can skip the extra roundtrip.
- [ ] Eliminate residual `TableQueries.TableColumnsSingle` fallbacks. Even after the cache refactor the snapshot logged 68 single-table column fetches. Expand the bulk loader to include cross-schema dependencies and normalise cache keys so these lookups can reuse the per-schema catalog data instead of hitting the database again.
