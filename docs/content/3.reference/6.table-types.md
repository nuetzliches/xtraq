...
`TableTypeMetadataProvider` queries the expanded snapshot artifacts under `.xtraq/snapshots` in a strict order:

1. `index.json` &rarr; `UserDefinedTableTypes` array
2. `.xtraq/snapshots/tabletypes/*.json` split files written by the snapshot writer
3. `.xtraq/snapshots/procedures/*.json` parameters (fallback inference when no dedicated UDTT file exists)

Legacy monolithic snapshot payloads are no longer read. After upgrading, run `xtraq snapshot` so the index and `tabletypes/` directory reflect the current database state. When running the CLI from a solution root or CI workspace, set `XTRAQ_PROJECT_PATH` (or pass `-p/--project-path`) so the resolver can locate the correct `.xtraq` directory before querying UDTT metadata.
...
Roadmap tracking for table type generation now lives in the central checklist ([Roadmap](/roadmap)).
